 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_martine_process_simple_like', 'martine_process_simple_like' );
add_action( 'wp_ajax_martine_process_simple_like', 'martine_process_simple_like' );
function martine_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'martine' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'martine' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'martine' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'martine' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'martine' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'martine' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'martine_show_user_likes' );
add_action( 'edit_user_profile', 'martine_show_user_likes' );
function martine_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'martine' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'martine' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
